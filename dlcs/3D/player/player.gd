...
func _physics_process(delta):
	...
	if (on_air):
		...
		if (Input.is_action_just_pressed("shoot")):
			...
			var bullet = preload("res://dlcs/3D/player/bullet.tscn").instance()
...
func _process(delta):
	...
	if timer > TIMER_LIMIT:
		timer = 0.0
		$CanvasLayer/Control/fps.set_text("FPS: " + str(Engine.get_frames_per_second()))

func _init():
	Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)


func _on_GameUI_FPSShow():
	$CanvasLayer/Control/fps.show()


func _on_GameUI_FPSHide():
	$CanvasLayer/Control/fps.hide()
