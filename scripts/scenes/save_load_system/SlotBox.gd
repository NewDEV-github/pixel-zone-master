...
func savebox(saveslotsdir : = saveslots_dir + "/") -> void:
	for x in $GridContainer.get_children():
		if filehandler.file_exists(saveslotsdir + x.name + '.png'):
			player.debug("slot exist, loading image")
			var img = Image.new()
...
func savepress(caller : String) -> bool:
	...
	if player.is_save_exits(caller):
		$GridContainer.hide()
	...
	var s = player.get_datetime_str()
	player.debug(s)
	filehandler.store_line(s)
	player.debug(["caller:", caller])
	player.savefile(caller)
