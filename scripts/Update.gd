...
func _ready():
	$ItemList.add_item("v0.7.2.0.7 - Current version")
	...
	if config_updates.file_exists(str(documents) + "/Pixel Zone/.data/updates/config/config.ini"):
		...
		$HTTPRequest5.request("https://github.com/MasterPolska123/pixel-zone-master/releases/tag/v0.7.2.0.8")
func _initialize():
	var exists = File.new()
...
#		$RichTextLabel.add_text("\nDetected version v0.7.2.0.7 to download")
	if exists.file_exists(str(documents) + "/Pixel Zone/.data/updates/version4.html") and exists.file_exists(str(documents) + "/Pixel Zone/.data/updates/config/config.ini"):
		$ItemList.add_item("v0.7.2.0.8")
		$RichTextLabel.add_text("\nDetected version v0.7.2.0.8 to download")
...
func _on_Button_pressed():
	$HTTPRequest4.set_download_file(str(documents) + "/Pixel Zone/.data/updates/3d.pck")
	$HTTPRequest4.request("https://github.com/MasterPolska123/pixel-zone-master/releases/download/%s/version0.7.2.0.5.txt" % version_to_download)
...
func _on_ItemList_item_selected(index):
	if index == 0:
		return
	if index == 1:
		version_to_download = "v0.7.2.0.5"
	if index == 2:
		version_to_download = "v0.7.2.0.6"
	if index == 3:
		version_to_download = "v0.7.2.0.7"
	if index == 4:
		version_to_download = "v0.7.2.0.8"
